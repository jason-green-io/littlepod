...
def updatefollowers():
    ...
    while True:
        ...
        updatedfollowers = {}
       	try:
            streamdata = json.loads(urllib.urlopen("https://api.twitch.tv/kraken/streams/greener_ca").read() )
            followerdata = json.loads(urllib.urlopen("https://api.twitch.tv/kraken/channels/greener_ca/follows/?limit=100").read() )
        except IOError:
        ...
        vanillabean.send(u"/scoreboard players set §6Followers §5Twitch " + str(len(followers)))
        vanillabean.send(u"/scoreboard players set §6Viewers §5Twitch " + str(viewers))
...
def sendtotwitch( string ):
    ...
    s.send("PRIVMSG #greener_ca :" + string + "\r\n")
